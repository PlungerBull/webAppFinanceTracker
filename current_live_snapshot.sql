...
CREATE OR REPLACE FUNCTION "public"."auto_set_cleared_flag"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
ALTER FUNCTION "public"."auto_set_cleared_flag"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."bulk_update_transactions"("p_transaction_ids" "uuid"[], "p_updates" "jsonb") RETURNS "jsonb"
...
ALTER FUNCTION "public"."bulk_update_transactions"("p_transaction_ids" "uuid"[], "p_updates" "jsonb") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."bulk_update_transactions"("p_transaction_ids" "uuid"[], "p_updates" "jsonb") IS 'BULK UPDATE with SACRED LEDGER VALIDATION: Updates multiple transactions atomically with field-level intent tracking. Only updates fields present in p_updates JSON. Validates category/account existence and ownership. Returns detailed success/error counts with per-transaction error messages for partial failure handling. Safe for concurrent edits - uses RLS checks within DEFINER mode.';
...
CREATE OR REPLACE FUNCTION "public"."check_reconciliation_account_match"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
...
ALTER FUNCTION "public"."check_reconciliation_account_match"() OWNER TO "postgres";


COMMENT ON FUNCTION "public"."check_reconciliation_account_match"() IS 'Sacred Ledger Hard Gate: Enforces that a transaction can only be linked to a reconciliation for the SAME bank account. Prevents mathematically impossible balance states.';



CREATE OR REPLACE FUNCTION "public"."check_reconciliation_date_overlap"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
...
ALTER FUNCTION "public"."check_reconciliation_date_overlap"() OWNER TO "postgres";


COMMENT ON FUNCTION "public"."check_reconciliation_date_overlap"() IS 'Prevents overlapping reconciliation date ranges for the same account. Ensures each time period is reconciled only once.';
...
CREATE OR REPLACE FUNCTION "public"."check_transaction_reconciliation_lock"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
...
ALTER FUNCTION "public"."check_transaction_reconciliation_lock"() OWNER TO "postgres";


COMMENT ON FUNCTION "public"."check_transaction_reconciliation_lock"() IS 'Semi-Permeable Lock: Prevents updates to amount_original, date, and account_id when transaction is linked to a completed reconciliation. Allows updates to category_id, description, notes, and exchange_rate for ongoing classification work.';
...
CREATE OR REPLACE FUNCTION "public"."get_reconciliation_summary"("p_reconciliation_id" "uuid") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."get_reconciliation_summary"("p_reconciliation_id" "uuid") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_reconciliation_summary"("p_reconciliation_id" "uuid") IS 'Calculate real-time reconciliation math: Difference = Ending Balance - (Beginning Balance + Linked Sum). Returns full summary for HUD display. Uses amount_home for accurate multi-currency support.';
...
CREATE OR REPLACE FUNCTION "public"."import_transactions"("p_user_id" "uuid", "p_transactions" "jsonb", "p_default_account_color" "text", "p_default_category_color" "text", "p_general_label" "text" DEFAULT 'General'::"text", "p_uncategorized_label" "text" DEFAULT 'Uncategorized'::"text") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
...
ALTER FUNCTION "public"."import_transactions"("p_user_id" "uuid", "p_transactions" "jsonb", "p_default_account_color" "text", "p_default_category_color" "text", "p_general_label" "text", "p_uncategorized_label" "text") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."import_transactions"("p_user_id" "uuid", "p_transactions" "jsonb", "p_default_account_color" "text", "p_default_category_color" "text", "p_general_label" "text", "p_uncategorized_label" "text") IS 'SACRED LEDGER: Import transactions from CSV with automatic account/category creation.
...
Features:
  ...
  - Extended timeout: 5-minute statement timeout for large imports (3000+ transactions)

Currency derived from account_id via trigger, ensuring transactions match account currency.';



CREATE OR REPLACE FUNCTION "public"."link_transactions_to_reconciliation"("p_reconciliation_id" "uuid", "p_transaction_ids" "uuid"[]) RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."link_transactions_to_reconciliation"("p_reconciliation_id" "uuid", "p_transaction_ids" "uuid"[]) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."link_transactions_to_reconciliation"("p_reconciliation_id" "uuid", "p_transaction_ids" "uuid"[]) IS 'Atomically link multiple transactions to a reconciliation. Validates account match and user ownership. Auto-sets cleared flag via trigger. Relies on check_reconciliation_account_match trigger for Sacred Ledger enforcement.';
...
CREATE OR REPLACE FUNCTION "public"."unlink_transactions_from_reconciliation"("p_transaction_ids" "uuid"[]) RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."unlink_transactions_from_reconciliation"("p_transaction_ids" "uuid"[]) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."unlink_transactions_from_reconciliation"("p_transaction_ids" "uuid"[]) IS 'Atomically unlink multiple transactions from a reconciliation. Prevents unlinking if reconciliation is completed. Auto-clears cleared flag via trigger.';
...
CREATE TABLE IF NOT EXISTS "internal"."transactions_currency_cleanup_backup" (
    "transaction_id" "uuid",
    "account_id" "uuid",
    "account_name" "text",
    "old_currency" "text",
...
ALTER TABLE "internal"."transactions_currency_cleanup_backup" OWNER TO "postgres";


COMMENT ON TABLE "internal"."transactions_currency_cleanup_backup" IS 'Audit trail of currency corrections made during Sacred Ledger migration (2025-12-27). Contains original values before trigger enforcement was implemented. This table can be safely dropped after verifying the migration was successful.';
...
COMMENT ON COLUMN "public"."transactions"."reconciliation_id" IS 'Links transaction to a reconciliation session. When set, cleared flag auto-sets to TRUE. When reconciliation.status=completed, this transaction becomes semi-permeable locked.';



COMMENT ON COLUMN "public"."transactions"."cleared" IS 'Auto-managed flag (do not set manually). TRUE when linked to reconciliation, FALSE otherwise. Used for UI indicators.';
...
CREATE TABLE IF NOT EXISTS "public"."reconciliations" (
    ...
    "status" "public"."reconciliation_status" DEFAULT 'draft'::"public"."reconciliation_status" NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "reconciliations_name_not_empty" CHECK (("char_length"("name") > 0)),
    CONSTRAINT "reconciliations_valid_date_range" CHECK (((("date_start" IS NULL) AND ("date_end" IS NULL)) OR (("date_start" IS NOT NULL) AND ("date_end" IS NOT NULL) AND ("date_end" >= "date_start"))))
);
...
COMMENT ON TABLE "public"."reconciliations" IS 'Reconciliation sessions tracking the "Contract of Truth" between bank statements and ledger. Each reconciliation defines beginning/ending balances and links transactions to prove the math.';



COMMENT ON COLUMN "public"."reconciliations"."status" IS 'draft: Reconciliation in progress, transactions can be linked/unlinked. completed: Reconciliation locked, linked transactions become immutable (amount/date/account frozen, but category/notes editable).';
...
CREATE TABLE IF NOT EXISTS "public"."user_settings" (
    ...
    "transaction_sort_preference" "text" DEFAULT 'date'::"text" NOT NULL,
    CONSTRAINT "user_settings_sort_preference_check" CHECK (("transaction_sort_preference" = ANY (ARRAY['date'::"text", 'created_at'::"text"]))),
...
COMMENT ON COLUMN "public"."user_settings"."transaction_sort_preference" IS 'User preference for transaction list sorting: date (Financial Ledger) or created_at (Activity Feed)';
...
ALTER TABLE ONLY "public"."categories"
    ADD CONSTRAINT "unique_user_category_name_per_parent" UNIQUE NULLS NOT DISTINCT ("user_id", "name", "parent_id");
...
CREATE INDEX "idx_reconciliations_user_account" ON "public"."reconciliations" USING "btree" ("user_id", "account_id");
...
CREATE INDEX "idx_transactions_cleared" ON "public"."transactions" USING "btree" ("cleared");
...
CREATE INDEX "idx_transactions_reconciliation_id" ON "public"."transactions" USING "btree" ("reconciliation_id");
...
COMMENT ON INDEX "public"."idx_transactions_user_created_at" IS 'Optimizes Activity Feed queries (ORDER BY created_at DESC) for Dual-Logic Ledger Architecture';
...
CREATE OR REPLACE TRIGGER "trigger_auto_set_cleared" BEFORE INSERT OR UPDATE OF "reconciliation_id" ON "public"."transactions" FOR EACH ROW EXECUTE FUNCTION "public"."auto_set_cleared_flag"();
...
CREATE OR REPLACE TRIGGER "trigger_check_reconciliation_account_match" BEFORE INSERT OR UPDATE OF "reconciliation_id", "account_id" ON "public"."transactions" FOR EACH ROW EXECUTE FUNCTION "public"."check_reconciliation_account_match"();



CREATE OR REPLACE TRIGGER "trigger_check_reconciliation_date_overlap" BEFORE INSERT OR UPDATE OF "date_start", "date_end", "account_id" ON "public"."reconciliations" FOR EACH ROW EXECUTE FUNCTION "public"."check_reconciliation_date_overlap"();



CREATE OR REPLACE TRIGGER "trigger_check_transaction_reconciliation_lock" BEFORE UPDATE ON "public"."transactions" FOR EACH ROW EXECUTE FUNCTION "public"."check_transaction_reconciliation_lock"();
...
ALTER TABLE ONLY "public"."reconciliations"
    ADD CONSTRAINT "reconciliations_account_id_fkey" FOREIGN KEY ("account_id") REFERENCES "public"."bank_accounts"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."reconciliations"
    ADD CONSTRAINT "reconciliations_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."transactions"
    ADD CONSTRAINT "transactions_reconciliation_id_fkey" FOREIGN KEY ("reconciliation_id") REFERENCES "public"."reconciliations"("id") ON DELETE SET NULL;
...
CREATE POLICY "Users can create their own reconciliations" ON "public"."reconciliations" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));
...
GRANT ALL ON FUNCTION "public"."auto_set_cleared_flag"() TO "anon";
GRANT ALL ON FUNCTION "public"."auto_set_cleared_flag"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."auto_set_cleared_flag"() TO "service_role";



GRANT ALL ON FUNCTION "public"."bulk_update_transactions"("p_transaction_ids" "uuid"[], "p_updates" "jsonb") TO "anon";
GRANT ALL ON FUNCTION "public"."bulk_update_transactions"("p_transaction_ids" "uuid"[], "p_updates" "jsonb") TO "authenticated";
GRANT ALL ON FUNCTION "public"."bulk_update_transactions"("p_transaction_ids" "uuid"[], "p_updates" "jsonb") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."check_reconciliation_account_match"() TO "anon";
GRANT ALL ON FUNCTION "public"."check_reconciliation_account_match"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."check_reconciliation_account_match"() TO "service_role";



GRANT ALL ON FUNCTION "public"."check_reconciliation_date_overlap"() TO "anon";
GRANT ALL ON FUNCTION "public"."check_reconciliation_date_overlap"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."check_reconciliation_date_overlap"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."check_transaction_reconciliation_lock"() TO "anon";
GRANT ALL ON FUNCTION "public"."check_transaction_reconciliation_lock"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."check_transaction_reconciliation_lock"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_reconciliation_summary"("p_reconciliation_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_reconciliation_summary"("p_reconciliation_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_reconciliation_summary"("p_reconciliation_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."import_transactions"("p_user_id" "uuid", "p_transactions" "jsonb", "p_default_account_color" "text", "p_default_category_color" "text", "p_general_label" "text", "p_uncategorized_label" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."import_transactions"("p_user_id" "uuid", "p_transactions" "jsonb", "p_default_account_color" "text", "p_default_category_color" "text", "p_general_label" "text", "p_uncategorized_label" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."import_transactions"("p_user_id" "uuid", "p_transactions" "jsonb", "p_default_account_color" "text", "p_default_category_color" "text", "p_general_label" "text", "p_uncategorized_label" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."link_transactions_to_reconciliation"("p_reconciliation_id" "uuid", "p_transaction_ids" "uuid"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."link_transactions_to_reconciliation"("p_reconciliation_id" "uuid", "p_transaction_ids" "uuid"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."link_transactions_to_reconciliation"("p_reconciliation_id" "uuid", "p_transaction_ids" "uuid"[]) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."unlink_transactions_from_reconciliation"("p_transaction_ids" "uuid"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."unlink_transactions_from_reconciliation"("p_transaction_ids" "uuid"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."unlink_transactions_from_reconciliation"("p_transaction_ids" "uuid"[]) TO "service_role";
...
GRANT ALL ON TABLE "internal"."transactions_currency_cleanup_backup" TO "anon";
GRANT ALL ON TABLE "internal"."transactions_currency_cleanup_backup" TO "authenticated";
GRANT ALL ON TABLE "internal"."transactions_currency_cleanup_backup" TO "service_role";
