-- Migration: Update create_account_with_currencies to use auth.uid()
-- Purpose: Remove p_user_id parameter and use auth.uid() internally for security
-- Created: 2025-01-16

-- Drop old function signatures
DROP FUNCTION IF EXISTS create_account_with_currencies(UUID, TEXT, JSONB);
DROP FUNCTION IF EXISTS create_account_with_currencies(TEXT, JSONB);

-- Create secure version that uses auth.uid() internally
...
BEGIN
  ...
  VALUES (v_user_id, trim(p_account_name))
...
GRANT EXECUTE ON FUNCTION create_account_with_currencies(TEXT, JSONB) TO authenticated;

-- Add comment for documentation
COMMENT ON FUNCTION create_account_with_currencies(TEXT, JSONB) IS 
  'Atomically creates a bank account with multiple currencies. Uses auth.uid() internally for security. Either all operations succeed or all fail.';