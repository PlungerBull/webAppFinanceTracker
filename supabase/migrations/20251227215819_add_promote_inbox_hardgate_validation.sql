-- Migration: Add Hard-Gate Validation to promote_inbox_item RPC
-- Purpose: Enforce server-side validation to prevent promotion without required fields
-- This ensures data integrity regardless of frontend validation state
...
-- Add comment explaining the hard-gate pattern
COMMENT ON FUNCTION "public"."promote_inbox_item" IS
'HARD-GATE VALIDATION: Explicitly validates all required fields (account_id, category_id, amount, description, date) before promotion. Prevents frontend bypasses and ensures ledger data integrity. Part of the Scratchpad → Clean Ledger architecture.';
